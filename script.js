...
loaderTypeEffect();
    ...
    // --- 3. SMOOTH NAV SCROLL (no more hide/show tabs or glitch flash) ---
    navLinks.forEach(link => {
        link.addEventListener('click', function(e) {
            ...
            isManualScroll = true; // Avoid the scroll-spy observer fighting the click for a moment
            ...
            setTimeout(() => { isManualScroll = false; }, 700);
        });
    });

    // --- 4. SCROLL-SPY (highlights the current nav item as you scroll) ---
    ...
    const observer = new IntersectionObserver((entries) => {
        if (isManualScroll) return; // Skip while a click-triggered scroll is in flight
    ...
    // --- 5. SCROLL REVEAL (gentle fade/slide-in the first time a section appears) ---
    const revealObserver = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                entry.target.classList.add('revealed');
                revealObserver.unobserve(entry.target); // Reveal once, don't re-hide on scroll back up
            }
        });
...
// Mouse-trail particle canvas removed — kept the site to one ambient
// background effect (matrix rain) plus the click ping for feedback,
// instead of stacking multiple constant-motion effects.
